...
uint256 hashGenesisBlock("0x30048a94e843408cb796b77be4a41cfdaf43609688027dd4b0d6b53da722ef37");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SoccerCoin: starting difficulty is 1 / 2^12
...
{
    // SoccerCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    else if (nHeight < 5651251)
    {nSubsidy = 125 * COIN;}
    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
   // nSubsidy >>= (nHeight / 840000); // SoccerCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 20 * 0.5 * 60; // SoccerCoin: 10 minutes
static const int64 nTargetSpacing = 0.5 * 60; // SoccerCoin: 0.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // SoccerCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // SoccerCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // SoccerCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x30048a94e843408cb796b77be4a41cfdaf43609688027dd4b0d6b53da722ef37");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "SoccerCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1394246224;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085272382;
        ...
        assert(block.hashMerkleRoot == uint256("0xf81018f84246101105142db6b4cc886ec835149a107dd116395b1bb50ef89916"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // SoccerCoin: increase each by adding 2 to bitcoin's value.
...
void static SoccerCoinMiner(CWallet *pwallet)
{
    printf("SoccerCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("SoccerCoin-miner");
    ...
    {
        printf("SoccerCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SoccerCoinMiner, pwallet));
}
